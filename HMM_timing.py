...
def init(context):
    ...
    # 2. 配置止损参数
    g.is_market_stop_loss_by_HMM = True
    g.is_day_stop_loss_by_HMM = False
    if g.is_market_stop_loss_by_HMM:
        # 配置价格止损判定指数，默认为上证指数，可修改为其他指数
        g.index_4_stop_loss_by_HMM = '000001.XSHG'
    # (1) 配置是否根据大盘历史价格止损
...
def before_trading(context):
    logger.info("---------------------------------------------")
    if g.is_market_stop_loss_by_HMM:
        g.is_day_stop_loss_by_HMM = is_day_stop_loss_by_hmm(context,\
                                                            g.index_4_stop_loss_by_HMM)

    if g.is_market_stop_loss_by_price:
        g.is_day_stop_loss_by_price = is_day_stop_loss_by_market_price(context, \
                                                                g.index_4_stop_loss_by_price)
...
def handle_bar(context, bar_dict):
    # HMM止损
    if g.is_day_stop_loss_by_HMM or g.is_day_stop_loss_by_price:
...
def log_param():
    ...
    if g.is_market_stop_loss_by_3_black_crows:
        logger.info("大盘三黑鸦止损判定指数: %s - %s" % (g.index_4_stop_loss_by_3_black_crows, \
                                              instruments(g.index_4_stop_loss_by_3_black_crows).symbol))
...
# 获取历史数据,用于建立HMM
def loaddata(security, bar_count=750):
    count_num = instruments(security).days_from_listed()
    data = history_bars(security, count_num, "1d", ['close', 'high', 'low', 'volume'])
    close = data['close']
    high = data['high']
    low = data['low']
    volume = np.array(data['volume'], dtype='f8')
    # 对数收益率 (LogReturn)
    LogRet_1 = np.array(np.diff(np.log(close)))
    # 平均真实波动范围
    ATR_14 = talib.ATR(high, low, close, timeperiod=13)
    ATR_14 = [x for x in ATR_14 if str(x) != 'nan']
    # 乘离率 (BIAS)
    average_price = talib.SMA(close, timeperiod=14)
    BIAS_14 = (close - average_price) / average_price  # 计算BIAS值
    BIAS_14 = [x for x in BIAS_14 if str(x) != 'nan']
    # 移动平均成交量 (VOSC)
    SHORTMA = talib.SMA(volume, timeperiod=12)
    LONGMA = talib.SMA(volume, timeperiod=26)
    ...
    BIAS_14 = BIAS_14[12:]
    VOSC = VOSC[:]
    A = np.column_stack([LogRet_1, ATR_14, BIAS_14, VOSC])
    return A

# 对单支股票建立隐马尔科夫模型HMM
def is_day_stop_loss_by_hmm(context, security):
    # 根据大盘指数的HMM模型，判断隐藏状态进行止损
    # 基于历史数据判定，因此若状态满足，则当天都不会变化
    is_stop_loss_by_hmm = False
    # 加载建模数据
    X = loaddata(security)
    # Make an HMM instance and execute fit
    model = GaussianHMM(n_components=context.n_components, covariance_type="full", n_iter=1000).fit([X])
    hist_state = model.predict(X)[-1]
    # 判断哪个是隐藏的上升、下跌和震荡状态
    # updown_condition = np.argsort(model.means_[:,0])
    buystate = [idx for (idx, val) in enumerate(model.means_[:, 0]) if val > 0]
    sellstate = [idx for (idx, val) in enumerate(model.means_[:, 0]) if val < 0]
    if hist_state in sellstate:
        # 当日第一次输出日志
        logger.info("==> 大盘HMM止损，%s指数昨日隐含状态收益为负, 状态: %f, 隐含收益率: %f" % (
            instruments(index).symbol, hist_state, model.means_[hist_state, 0]))
        is_stop_loss_by_hmm = True

    return is_stop_loss_by_hmm

def is_day_stop_loss_by_market_price(context, index):
    # 大盘指数前160日内最高价超过最低价2.2倍，则清仓止损
    # 基于历史数据判定，因此若状态满足，则当天都不会变化
    # 增加此止损，回撤降低，收益降低
    is_stop_loss_by_price = False
    ...
    if high_price > 2.2 * low_price \
        ...
        is_stop_loss_by_price = True

    return is_stop_loss_by_price
